...
#######################################################################
#######################################################################
######Add new function to calculate the maximum of likelihood function
evidence.alt <- function(x, N, mu,
...
# Prior distribution of q|H1: Gamma(rho1, nu1)
             ...
             ###Calculate for case-control
             prob.gene.CC[j] <- evidence.alt.CC(x = xI, N = N,
                                                gamma.mean = gamma.mean.CC[j], beta = beta.CC[j],
                                                rho1 = rho1, nu1 = nu1)$total



             ###Calculate for de novo
             prob.gene.dn[j] <- evidence.alt.dn(x = xI$dn, N = N$dn, mu = mu[j],
                                                gamma.mean = gamma.mean.dn[j], beta = beta.dn[j] )
...
#    prob.M1 <- exp(log(sum(c(prob.gene.CC, prob.gene.dn))))

#    prob.M1 <- exp(log(sum(c(prob.gene.CC))))
        prob.M1 <- exp(log(sum(c(prob.gene.dn))))
...
evidence.null <- function(x, N, mu,
    ...
    prob.gene.CC.null <- rep(1, k)

    prob.gene.dn.null <- rep(1, k)
...
# Prior distribution of q|H1: Gamma(rho1, nu1)
             ...
             prob.gene.CC.null[j] <- evidence.null.CC(x = xI, N = N,
                                                      rho0 = rho1, nu0 = nu1)$total


             ###Calculate for de novo

             prob.gene.dn.null[j] <- evidence.null.dn(x = xI$dn, N= N$dn, mu = mu[j])



         }

    #prob.M0 <- exp(log(sum(c(prob.gene.CC.null, prob.gene.dn.null))))

#        prob.M0 <- exp(log(sum(c(prob.gene.CC.null))))

    prob.M0 <- exp(log(sum(c(prob.gene.dn.null))))
